...
**Tests**: Test tasks are REQUIRED for constitution-critical behavior: phone mask,
LGPD consent, honeypot, payload shape, GTM events, webhook success/failure fallback,
WhatsApp redirect and any pure business logic. Additional tests follow the feature
specification.
...
- [ ] T010 Define centralized widget configuration for colors, phone number, webhook URL, GTM events and public selectors
...
- [ ] T013 [P] [US1] Unit tests for validation, formatting, payload and message generation in tests/unit/test_[name].py
...
- [ ] T018 [US1] Add validation, LGPD consent handling and honeypot handling
- [ ] T019 [US1] Add GTM event dispatch and safe no-dataLayer behavior
...
- [ ] T022 [P] [US2] Failure-path test for webhook timeout/error fallback in tests/integration/test_[name].py
...
- [ ] T029 [P] [US3] UX validation for desktop/mobile states in tests/integration/test_[name].py
...
- [ ] TXXX Validate webhook payload, GTM events and WhatsApp redirect against quickstart.md
- [ ] TXXX Validate contrast, responsive layout and no-blocking failure states
...
- Constitution-critical tests MUST be written and FAIL before implementation
...
```bash
# Launch all constitution-critical tests for User Story 1 together:
